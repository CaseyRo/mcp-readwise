...
Provide a single-call `reading_status` MCP tool that returns a structured snapshot of the user's relationship with their Readwise library — `this_window` recent activity, `evergreen_top` (intensity-ranked), `current_top` (raw-ranked, 30-day), `junk_drawer`, and raw `signal_density` numbers. Also expose a derived `reading_status` field on every `ReaderDocument` returned by direct lookup. Builds on the `engagement-scoring` capability.
