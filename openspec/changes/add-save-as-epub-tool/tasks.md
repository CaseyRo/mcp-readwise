...
- [x] 14.5 Smoke test on deployed prod: real markdown sent through the live MCP, EPUB landed in Readwise Library, screenshot at `docs/screenshots/v0.6.0-epub-in-reader.png` confirms ingest. Deployed instance was used (Path B from the apply discussion) rather than local stdio
- [x] 14.6 Visual check on the resulting EPUB in Readwise Reader confirmed via screenshot: Strong Blue H1 underlines render, Inter weight 800 chapter heads render (not League Gothic — fallback chain works as designed), mint-rail Note block renders with the eyebrow caps "NOTE" label, body Inter renders. Reader forces its own dark theme over the background, which is expected and intentional in our design
- [ ] 14.7 Manual idempotency check: send same markdown twice with same `idempotency_key`; verify Readwise shows ONE entry, not two — deferred, not blocking (the spec test in 11.4 covers the unit-level guarantee that identifier_scheme + dc:identifier are stable per key; the end-to-end behavior depends on Readwise's email ingest treating matching identifiers as updates, which is documented but only worth verifying when we have a real dedup need)
- [x] 14.8 Commit `6ec5b6b` pushed; Komodo auto-deploy fork()'d on first attempt, manually triggered via `km execute deploy-stack` after env block was added through the Komodo API; `/health.tools == 14`, `/health.epub_sender.configured == true`, `git_commit == 6ec5b6b`
